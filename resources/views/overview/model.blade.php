@php
    $string_limit = 40;
    $is_user_url = request()->routeIs('dashboard.view_user');
@endphp
...
<x-dashboard-layout>
    ...
    <x-filter-form action="{{ $is_user_url ? route('dashboard.view_user', $model) : route('dashboard.view_counter', $model) }}">
    ...
    <div class="p-4 bg-gray-50 dark:bg-gray-700 sm:rounded-lg shadow-sm text-gray-800 dark:text-gray-400 ">
        @if ($is_user_url)
            <h2 class="text-xl mb-4 dark:text-gray-300">Employee Info</h2>
            <div class="grid sm:grid-cols-2 gap-2">
                <div>
                    Employee Number : <span class="font-bold dark:text-gray-200">{{$model->number}}</span>
                </div>
                <div>
                    Name : <span class="font-bold dark:text-gray-200">{{ $model->name }}</span>
                </div>
                <div>
                    Position : <span class="font-bold dark:text-gray-200">{{ $model->position }}</span>
                </div>
                <div>
                    Short Name : <span class="font-bold dark:text-gray-200">{{ $model->sname }}</span>
                </div>
                <div>
                    Total Feedback : <span class="font-bold dark:text-gray-200">{{ $count }}</span>
                </div>
                <div>
                    Average Rating : <span class="font-bold dark:text-gray-200">{{ number_format($average, 2) }} / 5</span>
                </div>
            </div>
        @else
            <h2 class="text-xl mb-4 dark:text-gray-300">Counter Info</h2>
            <div class="grid sm:grid-cols-2 gap-2">
                <div>
                    Branch : <span class="font-bold dark:text-gray-200">{{ $model->branch->name }}</span>
                </div>
                <div>
                    Counter No. : <span class="font-bold dark:text-gray-200">{{ $model->number }}</span>
                </div>
                <div>
                    Total Feedback : <span class="font-bold dark:text-gray-200">{{ $count }}</span>
                </div>
            ...
            <x-table>
                ...
                <x-slot name="tbody">
                    ...
                    <tr>
                        ...
                        <x-table-body-col>
                            @if ($is_user_url)
                                {{ $feedback->loginsession->counter->number }}
                                <div class="text-xs">
                                    {{ $feedback->loginsession->counter->branch->name }}
                                </div>
                            @else
                                {{ $feedback->loginsession->counter_user->number }}
                            @endif
                        </x-table-body-col>
                        <x-table-body-col>
                ...
                <div class="mt-3 text-center sm:mt-0 sm:mx-4 sm:text-left divide-y divide-gray-400 dark:divide-gray-100">
                    <div class="flex justify-between items-center mb-2">
                        <h3 class="text-xl leading-6 font-medium dark:text-gray-200 text-gray-900" id="modal-title">
                            Feedback <span x-html="contentData.id"></span>
                        </h3>
                    ...
                    <div class="mb-2">
                        @if (!$is_user_url)
                            <x-modal-row title="Employee Number" html="contentData.loginsession?.counter_user?.number"/>
                            <x-modal-row title="Name" html="contentData.loginsession?.counter_user?.name"/>
                        @else
                            <x-modal-row title="Counter Number" html="contentData.loginsession?.counter?.number"/>
                            <x-modal-row title="Branch" html="contentData.loginsession?.counter?.branch.name"/>
                        @endif
                    </div>
                    <div class="mb-2">
